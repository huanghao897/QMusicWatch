...
让 480×480 方屏手表上的搜索、播放队列和歌词更紧凑、明确且可控。队列拖动时实时换位，喜欢与歌单按歌曲勾选导入；歌词按连续时间平滑填充，切换当前行时淡入、缩放并轻微左移，任何单句都不得换行。
...
- 当前歌词行明显大于其他行并以淡入、缩放、左移动画进入焦点；QRC 可用时按服务端字级时间连续填充，普通 LRC 按相邻行时间连续填充；无法安全解析的 QRC 不得使歌词为空
- 原文和翻译均为单行，时间占独立窄列；长句自动缩小字号，极端长度只允许截断，不允许换到第二行
